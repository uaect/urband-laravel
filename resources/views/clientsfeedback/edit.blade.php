@extends('layouts.app', ['title' => __('ClientFeedback Management')])
...
@endsection
@section('content')
    @include('clientsfeedback.partials.header', ['title' => __('Edit Client')])
                    ...
                    <div class="card-header bg-white border-0">
                        <div class="row align-items-center">
                            ...
                            <div class="col-4 text-right">
                                <a href="{{ route('clientfeedback.index') }}" class="btn btn-sm btn-primary"><span><i class="fas fa-arrow-left"></i></span>
                                    <span class="btn-inner--text">{{ __('Back to list') }}</span></a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{{ route('clientfeedback.update', $clientsfeedback) }}" autocomplete="off" enctype="multipart/form-data">
                            @csrf
                            @method('put')
                            ...
                            <div class="col-md-6">
                                <div class="form-group{{ $errors->has('title') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="title" id="input-title"
                                        ...
                                        placeholder="{{ __('Title') }}" value="{{ old('title', $clientsfeedback->title) }}" required autofocus>
                            ...
                            <div class="col-md-12">
                                <div class="form-group{{ $errors->has('description') ? ' has-danger' : '' }}">
                                    ...
                                    <textarea name="description"
                                        ...
                                        placeholder="{{ __('Description') }}" autofocus rows="10">{{ old('description', $clientsfeedback->description) }}</textarea>
                        ...
                        <div class="row">
                            ...
                            <?php if(@$clientsfeedback) { ?>
                            <img src="<?php if(@$clientsfeedback) {?>/storage/{{ @$clientsfeedback->contentImage() }}<?php } ?>" id="preview" class="img-thumbnail">
                            <?php } ?>
                            </div>
                        </div>
                        <div class="row">
