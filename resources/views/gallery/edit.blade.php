...
@endsection
@section('content')
    @include('gallery.partials.header', ['title' => __('Edit Gallery')])
                    ...
                    <div class="card-header bg-white border-0">
                        <div class="row align-items-center">
                            ...
                            <div class="col-4 text-right">
                                <a href="{{ route('gallery.index') }}" class="btn btn-sm btn-primary"><span><i class="fas fa-arrow-left"></i></span>
                                    <span class="btn-inner--text">{{ __('Back to list') }}</span></a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{{ route('gallery.update', $gallery) }}" autocomplete="off" enctype="multipart/form-data" id="gallery-form">
                            @csrf
                            ...
                            <div class="col-md-6">
                                <div class="form-group{{ $errors->has('title') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="title" id="input-title"
                                        ...
                                        placeholder="{{ __('Title') }}" value="{{ old('title', $gallery->title) }}" required autofocus>
                            ...
                            <div class="col-md-6">
                                <div class="form-group{{ $errors->has('sub_title') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="sub_title" id="input-sub_title"
                                        ...
                                        placeholder="{{ __('Sub Title') }}" value="{{ old('sub_title', $gallery->sub_title) }}" required>
                            ...
                            <div class="col-md-4">
                                <div class="form-group{{ $errors->has('category') ? ' has-danger' : '' }}">
                                    ...
                                    @foreach($category as $cat)
                                        <option value="{{$cat->title}}" {{($gallery->category==$cat->title)?'selected':''}}>{{$cat->title}}</option>
                                        @endforeach
                                    </select>
...
var uploadedDocumentMap = {}
  Dropzone.options.documentDropzone = {
    ...
    init: function () {
      ...
      @if(isset($gallery) && $gallery->galleryImages)
        var files =
          {!! json_encode($gallery->galleryImages) !!}
        for (var i in files) {
          var file = files[i]
          console.log(file);
          ...
          $('#gallery-form').append('<input type="hidden" name="document[]" value="' + file.image + '">')
        }
      @endif
    ...
    removedfile: function (file) {
      ...
      var _ref;
      $('#gallery-form').append('<input type="hidden" name="delete_files[]" value="' + name + '">')
