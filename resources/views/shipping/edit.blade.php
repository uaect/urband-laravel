...
@section('content')
    @include('shipping.partials.header', ['title' => __('Edit Shipping Charges')])
                    ...
                    <div class="card-header bg-white border-0">
                        <div class="row align-items-center">
                            ...
                            <div class="col-4 text-right">
                                <a href="{{ route('shipping.index') }}" class="btn btn-sm btn-primary"><span><i class="fas fa-arrow-left"></i></span>
                                    <span class="btn-inner--text">{{ __('Back to list') }}</span></a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{{ route('shipping.update', $shipping) }}" autocomplete="off" enctype="multipart/form-data">
                            @csrf
                            @method('put')
                        ...
                        <div class="row">
                            ...
                            <div class="form-group{{ $errors->has('cities') ? ' has-danger' : '' }}">
                                    ...
                                    <select
                                        ...
                                        <option value="{{$city->id}}" {{$shipping->cities==$city->id?'selected':''}}>{{$city->location}}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-4">
                                <div class="form-group{{ $errors->has('amount') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="amount" id="input-amount"
                                        ...
                                        placeholder="{{ __('Price') }}" value="{{ old('amount', $shipping->amount) }}" required autofocus>
                            ...
                            <div class="col-md-4">
                                <div class="form-group{{ $errors->has('estimate_time') ? ' has-danger' : '' }}">
                                    ...
                                    <select
                                        ...
                                        <option class="item" value="<?php echo $day; ?>" {{($shipping->estimate_time==$day)?'selected':''}}><?php echo $day; ?> Day<?php if($j!=1) echo 's'; ?></option>
                                        <?php } ?>
                                    </select>
