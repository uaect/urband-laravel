...
@endsection
@section('content')
    @include('clients.partials.header', ['title' => __('Edit Client')])
                    ...
                    <div class="card-header bg-white border-0">
                        <div class="row align-items-center">
                            ...
                            <div class="col-4 text-right">
                                <a href="{{ route('client.index') }}" class="btn btn-sm btn-primary"><span><i class="fas fa-arrow-left"></i></span>
                                    <span class="btn-inner--text">{{ __('Back to list') }}</span></a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{{ route('client.update', $client) }}" autocomplete="off" enctype="multipart/form-data">
                            @csrf
                            @method('put')
                            ...
                            <div class="col-md-6">
                                <div class="form-group{{ $errors->has('title') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="title" id="input-title"
                                        ...
                                        placeholder="{{ __('Title') }}" value="{{ old('title', $client->title) }}" required autofocus>
                        ...
                        <div class="row">
                            ...
                            <?php if(@$client) { ?>
                            <img src="<?php if(@$client) {?>/storage/{{ @$client->contentImage() }}<?php } ?>" id="preview" class="img-thumbnail">
                            <?php } ?>
                            </div>
                        </div>
                        <div class="row">
